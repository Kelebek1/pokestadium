 ...
 */
static Acmd* _pullSubFrame(N_PVoice* pv, s16* inp, s16* outp, s32 outCount, Acmd* p);
...
static Acmd* _pullSubFrame(N_PVoice* filter, s16* inp, s16* outp, s32 outCount, Acmd* p) {
    ...
    if (e->em_first) {
         ...
         */
        e->em_ltgt = (e->em_volume * n_eqpower[e->em_pan]) >> 15;
        e->em_lratm = _getRate((f64)e->em_cvolL, (f64)e->em_ltgt, e->em_segEnd, &(e->em_lratl));
        e->em_rtgt = (e->em_volume * n_eqpower[N_EQPOWER_LENGTH - e->em_pan - 1]) >> 15;
        e->em_rratm = _getRate((f64)e->em_cvolR, (f64)e->em_rtgt, e->em_segEnd, &(e->em_rratl));
